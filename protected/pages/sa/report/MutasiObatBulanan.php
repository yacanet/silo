    ...
    public function populateData ($search=false) {
        ...
        while (list($k,$v)=each($r)) {
            ...
            if ($v['kode_obat']=='') {
                ...
                $penerimaan=$this->DB->getSumRowsOfTable('qty',"master_sbbm msb,detail_sbbm dsb WHERE msb.idsbbm=dsb.idsbbm AND dsb.idobat=$idobat AND harga=$harga AND status='complete' AND DATE_FORMAT(msb.tanggal_sbbm,'%Y-%m')='$bulantahun'");
                $v['penerimaan']=$penerimaan;
                $v['harga_penerimaan']=$this->Obat->toRupiah($penerimaan*$harga);                
                $pengeluaran=$this->DB->getSumRowsOfTable('pemberian',"master_sbbk msk,detail_sbbk dsk WHERE msk.idsbbk=dsk.idsbbk AND dsk.idobat=$idobat AND harga=$harga AND status='complete' AND DATE_FORMAT(msk.tanggal_sbbk,'%Y-%m')='$bulantahun'");
